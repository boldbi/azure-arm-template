...
$(document).ready(function () {
    ...
    $("#new-password").bind("keyup", function () {
        if ($("#new-password").val() == $("#confirm-password").val()) {
            ...
            $("#confirm-password").closest('div').next("div").find("span").html(window.IdP.App.LocalizationContent.PasswordMismatch).css("display", "block");
        }
    ...
    $('.change-password-form').validate({
        ...
        messages: {
            "old-password": {
                required: window.IdP.App.LocalizationContent.OldPasswordValidator
            },
            "new-password": {
...
function createPasswordPolicyRules() {
    if ($("#new-password").val() != '' && $("#new-password").next("ul").length == 0) {
        ...
        $("#password_policy_rules").append("<li id='p_policy_heading'>" + window.IdP.App.LocalizationContent.PasswordRule1 + "</li>")
        $("#password_policy_rules").append("<li id='p_policy_length'><span class='su su-close'></span>" + window.IdP.App.LocalizationContent.PasswordRule2 + "</li>")
        $("#password_policy_rules").append("<li id='p_policy_uppercase'><span class='su su-close'></span>" + window.IdP.App.LocalizationContent.PasswordRule3 + "</li>")
        $("#password_policy_rules").append("<li id='p_policy_lowercase'><span class='su su-close'></span>" + window.IdP.App.LocalizationContent.PasswordRule4 + "</li>")
        $("#password_policy_rules").append("<li id='p_policy_number'><span class='su su-close'></span>" + window.IdP.App.LocalizationContent.PasswordRule5 + "</li>")
        $("#password_policy_rules").append("<li id='p_policy_specialcharacter'><span class='su su-close'></span>" + window.IdP.App.LocalizationContent.PasswordRule6 + "</li>")
...
function onChangePasswordClick() {
    ...
    if (isValid && $("#new-password").val() != $("#confirm-password").val()) {
        $("#confirm-password-validate").html(window.IdP.App.LocalizationContent.PasswordMismatch);
    ...
    doAjaxPost('POST', updatepasswordUrl, { oldpassword: $("#old-password").val(), newpassword: $("#new-password").val(), confirmpassword: $("#confirm-password").val(), returnurl: $("#hidden-return-url").val() },
                 function (result) {
                     ...
                     if (!result.Data.status && result.Data.key == "password") {
                         ...
                         ShowWaitingProgress("#content-area", "hide");
                         WarningAlert(window.IdP.App.LocalizationContent.UpdatePassword, window.IdP.App.LocalizationContent.PasswordFailure, 7000);
                     }
                     else {
                         if (result.Data.isTenantUserEdit) {
                             ...
                             ShowWaitingProgress("#content-area", "hide");
                             SuccessAlert(window.IdP.App.LocalizationContent.UpdatePassword, window.IdP.App.LocalizationContent.PasswordSuccess, 7000);
                         }
                     }
                 }
            );
