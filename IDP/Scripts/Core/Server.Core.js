...
function validateUserName(userName) {
    if (/\s/g.test(userName)) {
        return { isValid: false, message: window.IdP.App.LocalizationContent.HasWhiteSpace };
    }
    if (/[^a-zA-Z0-9]/.test(userName)) {
        return { isValid: false, message: window.IdP.App.LocalizationContent.UserNameSpecialCharacterValicator };
    }
...
function messageBox(messageIcon, messageHeader, messageText, type, successCallback, errorCallback, width, height, maxHeight) {
    ...
    if (type == "error") {
        ...
        if (successCallback != undefined) {
            successButton = $("<input type='button' class='critical-action-button pull-right' value='" + window.IdP.App.LocalizationContent.YesButton + "'></input>");
            successButton.bind("click", $.proxy(getFnObj(successCallback), window));
        }
        if (errorCallback != undefined) {
            errorButton = $("<input type='button' class='secondary-button pull-right' value='" + window.IdP.App.LocalizationContent.NoButton + "'></input>");
        ...
        var closeIcon;
        if (successCallback != undefined) {
            successButton = $("<input type='button' class='secondary-button' value='" + window.IdP.App.LocalizationContent.OKButton + "'></input>");
            successButton.bind("click", $.proxy(getFnObj(successCallback), window));
            closeIcon = $('<span class="su su-close"></span>');
...
function deleteUserAvatar() {
    ...
    doAjaxPost('POST', deleteavatarUrl, { id: $("#userId").val() },
        function (result) {
            ...
            if (result.status) {
                SuccessAlert(window.IdP.App.LocalizationContent.DeleteAvatar, window.IdP.App.LocalizationContent.DeleteAvatarSuccess, 7000);
                ...
                WarningAlert(window.IdP.App.LocalizationContent.DeleteAvatarTitle, window.IdP.App.LocalizationContent.DeleteAvatarError, 7000);
            }
...
function GridLocalization() {
    ej.Grid.Locale["en-US"] = {
        EmptyRecord: window.IdP.App.LocalizationContent.NoRecords,
        StringMenuOptions: [{ text: window.IdP.App.LocalizationContent.SearchKeyStart, value: "StartsWith" }, { text: window.IdP.App.LocalizationContent.SearchKeyEnd, value: "EndsWith" }, { text: window.IdP.App.LocalizationContent.SearchKeyContanins, value: "Contains" }, { text: window.IdP.App.LocalizationContent.SearchKeyEqual, value: "Equal" }, { text: window.IdP.App.LocalizationContent.SearchKeyNotEqual, value: "NotEqual" }],
        FilterMenuCaption: window.IdP.App.LocalizationContent.SearchValue,
        Filter: window.IdP.App.LocalizationContent.Search,
        Clear: window.IdP.App.LocalizationContent.ClearSearch
    };
    ej.Pager.Locale["en-US"] = {
        pagerInfo: window.IdP.App.LocalizationContent.PageCount,
        firstPageTooltip: window.IdP.App.LocalizationContent.FirstPage,
        lastPageTooltip: window.IdP.App.LocalizationContent.LastPage,
        nextPageTooltip: window.IdP.App.LocalizationContent.NextPage,
        previousPageTooltip: window.IdP.App.LocalizationContent.PreviousPage
    };
}
