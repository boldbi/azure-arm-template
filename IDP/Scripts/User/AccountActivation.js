$(document).ready(function () {
    $("#Save_Activate").on('click', function () {
        if ($("#new_password").val() == '') {
            $("#password_validate").html(window.IdP.App.LocalizationContent.PasswordValidator);
            return;
        }
        if ($("#new_password").val() != $("#Confirm_password").val()) {
            $("#re_password_validate").html(window.IdP.App.LocalizationContent.PasswordsMismatch);
            return;
        }
        $.ajax({
            type: "POST",
