...
function MakeFlyDeleteUsers() {
    ...
    doAjaxPost("POST", deleteFromUserListUrl, values, function (data) {
        if (data.status) {
            parent.messageBox("su-delete", "Delete User(s)", "User(s) has been deleted successfully.", "success", function () {
                var count = parent.$("#user-count-text").val();
            ...
            parent.messageBox("su-delete", "Delete User(s)", "Failed to delete user(s), please try again later.", "success", function () {
                $("#user-delete-confirmation").ejDialog("close");
                parent.onCloseMessageBox();
            });
        }
    }, function () {
        parent.messageBox("su-delete", "Delete User(s)", "Failed to delete user(s), please try again later.", "error", function () {
            $("#user-delete-confirmation").ejDialog("close");
            parent.onCloseMessageBox();
        });
    });
...
function deleteSingleUser() {
    ...
    doAjaxPost("POST", deleteSingleFromUserListUrl, "UserId=" + userId, function (data) {
        if (data.status) {
            ...
            parent.messageBox("su-delete", "Delete User", "User has been deleted successfully.", "success", function () {
                parent.onCloseMessageBox();
            });
            onConfirmDeleteUser("1");
            $("#singleuser-delete-confirmation").ejDialog("close");
        } else {
            parent.messageBox("su-delete", "Delete User", "Failed to delete user, please try again later.", "error", function () {
                parent.onCloseMessageBox();
            });
        }
    });
