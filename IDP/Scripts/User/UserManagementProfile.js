...
$(document).ready(function () {
    ...
    $('.edit-user-profile-form').validate({
        ...
        messages: {
            ...
            "user-firstname": {
                isRequired: window.IdP.App.LocalizationContent.FirstNameValidatorTitle
            }
        }
    });
    ...
    $("#new-password").bind("keyup", function () {
        if ($("#new-password").val() == $("#confirm-password").val()) {
            ...
            $("#confirm-password").closest('div').next("div").find("span").html(window.IdP.App.LocalizationContent.PasswordsMismatch).css("display", "block");
        }
    ...
    $('.change-password-form').validate({
        ...
        messages: {
            ...
            "confirm-password": {
                required: window.IdP.App.LocalizationContent.ConfirmPasswordValidator,
                equalTo: window.IdP.App.LocalizationContent.PasswordsMismatch
            }
        }
    });
            ...
            success: function (data) {
                ...
                if ($.type(data) == "object") {
                    if (data.Data.result == "success" && data.Data.type == 1) {
                        ...
                        CheckMailSettingsAndNotify(window.IdP.App.LocalizationContent.ActivationCodeNotSend + " <a href='" + emailSettingsUrl + "'>" + window.IdP.App.LocalizationContent.ConfigureMail + "</a>", $("#alert-message"), window.IdP.App.LocalizationContent.ActivationCodeSent);
                    }
                    else if (data.Data.result == "success") {
                        $("#inactive-user").hide();
                        $('<span id="active-user">' + window.IdP.App.LocalizationContent.Active + '</span>').insertAfter('#inactive-user').addClass("content-size status");
                        $("#user-status").val("true");
                        $("#status-user").val("Active");
                        $("#" + id).attr("disabled", true);
                        ValidateMailSettingsAndShowToast("", $("#alert-message"), window.IdP.App.LocalizationContent.UserActivated);
                    }
                }
            }
        });
            ...
            success: function (data) {
                ...
                if ($.type(data) == "object") {
                    if (data.Data.result == "success") {
                        CheckMailSettingsAndNotify(window.IdP.App.LocalizationContent.ActivationCodeNotSend + " <a href='" + emailSettingsUrl + "'>" + window.IdP.App.LocalizationContent.ConfigureMail + "</a>", $("#alert-message"), window.IdP.App.LocalizationContent.ActivationCodeResent);
