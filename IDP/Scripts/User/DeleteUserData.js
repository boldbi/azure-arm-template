...
function onConfirmCloseAccount() {
    ...
    $.ajax({
        ...
        success: function (result) {
            if (result.Success) {
                $(".modal-footer").hide();
                $(".delete-info").addClass("left-align").text(window.IdP.App.LocalizationContent.UserAccountClosedSuccess);
                ...
                WarningAlert(window.IdP.App.LocalizationContent.UserAccountClosed, window.IdP.App.LocalizationContent.UserAccountClosedFailure, 7000);
            }
        }
    });
}