$(document).ready(function () {
    $("#download-data-action-popup").ejDialog({
        ...
        allowDraggable: true,
        enableResize: false,
        title: window.IdP.App.LocalizationContent.DownloadData,
        enableModal: true,
        showHeader: false
    });
    ...
    $.ajax({
        ...
        success: function (result) {
            if (result.status) {
                ...
                WarningAlert(window.IdP.App.LocalizationContent.DownloadData, window.IdP.App.LocalizationContent.DownloadDataFailure, 7000);
            }
...
$(document).on("click", "#copy-password", function () {
    ...
    if (/Safari/.test(navigator.userAgent) && /Apple Computer/.test(navigator.vendor)) {
        ...
        copyBtn.tooltip("hide").attr("data-original-title", window.IdP.App.LocalizationContent.Copysuccess).tooltip("fixTitle").tooltip("show");
        setTimeout(function () {
            copyBtn.attr("data-original-title", window.IdP.App.LocalizationContent.PasswordCopy);
            copyBtn.tooltip();
        }, 3000);
    }
});