...
$(document).ready(function () {
    ...
    $("#card-form-id").validate({
        ...
        messages: {
            "pay-form-info-fullname": {
                isRequired: window.Server.App.LocalizationContent.FullnameValidator
            },
            "pay-form-info-email": {
                isRequired: window.Server.App.LocalizationContent.EmailValidator,
                isValidEmail: window.Server.App.LocalizationContent.IsValidEmail
            },
            "pay-form-info-address1": {
                isRequired: window.Server.App.LocalizationContent.AddressValidator
            },
            "pay-form-info-city": {
                isRequired: window.Server.App.LocalizationContent.CityValidator
            },
            "pay-form-info-country": {
                isRequired: window.Server.App.LocalizationContent.CountryValidator
            },
            "pay-form-info-state": {
                isRequired: window.Server.App.LocalizationContent.StateValidator
            },
            "pay-form-info-zip": {
                isRequired: window.Server.App.LocalizationContent.ZipValidator
            },
            "pay-form-info-card-name": {
                isRequired: window.Server.App.LocalizationContent.NameonCardValidator
            }
        }
    });
});
...
$(document).on("click", ".update-billing-address", function () {
    if (isBillingAddressChanged) {
        ...
        $("#validation-detail>.validation-error").text(window.Server.App.LocalizationContent.BillingAddressNotChanged);
    }
});
    ...
    $.ajax({
        ...
        success: function (result) {
            ...
            if (result.Status) {
                ...
                SuccessAlert(window.Server.App.LocalizationContent.UpdateBillingAddressTitle, window.Server.App.LocalizationContent.UpdateBillingAddressSuccess, 2000);
            } else {
                WarningAlert(window.Server.App.LocalizationContent.UpdateBillingAddressTitle, window.Server.App.LocalizationContent.UpdateBillingAddressFailed, 2000);
            }
        }
    });
}
...
function retryPayment() {
    ...
    messageBox("", window.Server.App.LocalizationContent.RetryPaymentTitle, window.Server.App.LocalizationContent.RetryPaymentWait, "success", function () {
        onCloseMessageBox();
    ...
    $.ajax({
        ...
        success: function (result) {
            ...
            if (result.Status) {
                messageBox("", window.Server.App.LocalizationContent.RetryPaymentTitle, window.Server.App.LocalizationContent.RetryPaymentSuccess, "success", function () {
                    onCloseMessageBox();
                    location.reload();
                }, undefined, "430px", "140px", undefined, "retry-payment-messageBox");
            } else {
                if (result.Data == "Something went wrong, try again.") {
                    messageBox("", window.Server.App.LocalizationContent.RetryPaymentTitle, window.Server.App.LocalizationContent.RetryPaymentFailDescription3, "success", function () {
                        onCloseMessageBox();
                    }, undefined, "420px", undefined, undefined, "retry-payment-messageBox");
                }
                else {
                    messageBox("", window.Server.App.LocalizationContent.RetryPaymentTitle, window.Server.App.LocalizationContent.RetryPaymentFailDescription1 + "<br/>" + result.Data + " – <a href='" + helpSiteBaseUrl + "/site-settings/payments/error-codes' target='_blank'>" + window.Server.App.LocalizationContent.RetryPaymentFailDescription2, "success", function () {
                        onCloseMessageBox();
            ...
            $.ajax({
                ...
                success: function (data) {
                    if (!data.Status) {
                        ...
                        hideWaitingPopup("server-app-container");
                        SuccessAlert(window.Server.App.LocalizationContent.Payments, window.Server.App.LocalizationContent.AddPaymentDetailSuccess, 7000);
                        card.clear();
                        cvc.clear();
                        exp.clear();
                        window.location.reload();
