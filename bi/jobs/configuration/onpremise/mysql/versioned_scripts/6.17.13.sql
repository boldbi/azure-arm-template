-- BOLD_UPGRADE_RERUN_SAFE: true
-- BOLD_UPGRADE_IDEMPOTENT_VALIDATED: true
-- BOLD_UPGRADE_STATEMENT_SEPARATOR: $$

SET @boldbi_itemlogtype_count := (
    SELECT COUNT(*)
    FROM {database_name}.BOLDBI_ItemLogType
    WHERE Name = 'Downloaded'
);

SET @boldbi_itemlogtype_valid := (
    SELECT COUNT(*)
    FROM {database_name}.BOLDBI_ItemLogType
    WHERE Name = 'Downloaded'
      AND IsActive = 1
);

SET @boldbi_sql := IF(
    @boldbi_itemlogtype_count = 0,
    'INSERT INTO {database_name}.BOLDBI_ItemLogType (Name, IsActive) VALUES (''Downloaded'', 1)',
    IF(@boldbi_itemlogtype_count = @boldbi_itemlogtype_valid, 'SELECT 1', 'CALL BOLD_UPGRADE_VALIDATION_FAILED()')
);

PREPARE boldbi_stmt FROM @boldbi_sql;
EXECUTE boldbi_stmt;
DEALLOCATE PREPARE boldbi_stmt;
$$
