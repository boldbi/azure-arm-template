...
mkdir -p "$clientlibrary"
echo "$INSTALL_OPTIONAL_LIBS" >> "$clientlibrary/optional-libs.txt"

log "Installing optional libraries..."
bash install-optional.libs.sh

if [[ ${BOLD_SERVICES_PV_USED:-} == true ]]; then
    if [ -f "/application/utilities/customwidgetupgrader/CustomWidgetUpgrader.dll" ]; then
        log "Running Custom Widget Upgrader..."
...
move_chromium_to_destination() {
    ...
    chrome_destination="/application/bi/dataservice/puppeteer/Linux-901912/chrome-linux"
    ...
    if [ -f "$chromium_path" ]; then
        ...
        log "Chromium package moved and renamed to 'chrome' successfully"
    fi
}
