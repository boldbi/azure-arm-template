...
CREATE TABLE SyncDS_ApiKeyDetails (
	...
	IsApiKeyViewed smallint NOT NULL,
    IsActive smallint NOT NULL)
;
...
INSERT into SyncDS_Group (Name,Description,Color,IsolationCode,ModifiedDate,DirectoryTypeId,IsAdminGroup,IsActive) VALUES (N'System Administrator','Has administrative rights for the dashboards','#ff0000',null,now() at time zone 'utc', 1, 1, 1)
;
...
-- File: boldbi_create_indexes_all_postgresql.sql
-- Purpose: Create btree indexes for all BoldBI tables (PostgreSQL).
-- Notes:
-- - Works with any schema if you set search_path first (see note above).
-- - Index names are preserved exactly (quoted); tables/columns are lowercase.
-- - PostgreSQL 11+ required for INCLUDE and partial indexes.
-- -Optional: Add CONCURRENTLY after CREATE INDEX for online builds (run outside a transaction).
