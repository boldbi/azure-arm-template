...
homeApp.controller("NotificationController", function ($scope, $window, $http) {
    ...
    function enableWaitingPopup(id, containerHeight) {
        ...
        var popupImage = $("<img>");
        popupImage.attr({ "src": dashboardServerResourceUrl + "/images/filter_loader.gif" });
    ...
    $scope.getInitDays = function (days) {
        if (days < 30) {
            if (days == 1) {
                return window.Server.App.LocalizationContent.Day;
            }
            else {
                return days + window.Server.App.LocalizationContent.Days;
            }
        }
        if (days > 30 && days < 365) {
            if (days < 60) {
                return window.Server.App.LocalizationContent.Month;
            }
            else {
                return Math.floor(days / 30) + window.Server.App.LocalizationContent.Months;
            }
        }
        if (days > 365) {
            if (days < 365) {
                return window.Server.App.LocalizationContent.Year;
            }
            else {
                return Math.floor(days / 365) + window.Server.App.LocalizationContent.Years;
            }
        }
    }
