...
function fnActionBegin(args) {
    var itemGridWaitingPopupTemplateId = createLoader("user_grid");
    ...
    $.ajax({
        ...
        success: function (result) {
            if (result.Data !== null) {
                ...
                var title = synchronizeSelectedUsersUrl.indexOf("azure") > 0 ? window.Server.App.LocalizationContent.AzureADSync : window.Server.App.LocalizationContent.ADSync;
        ...
        error: function () {
            ...
            $(".message-box-btn-holder").addClass("msg-footer-height");
            var title = synchronizeSelectedUsersUrl.indexOf("azure") > 0 ? window.Server.App.LocalizationContent.AzureADSync : window.Server.App.LocalizationContent.ADSync;
            parent.messageBox("su-user-1", title, window.Server.App.LocalizationContent.DatabaseUserSynchronizeFailed, "success", function () {               
                parent.onCloseMessageBox();
            });
            hideWaitingPopup("server-app-container");
        }
...
function SynchronizeDatabaseUsers() {
    ...
    $.ajax({
        ...
        success: function (result) {
            $("#messageBox_wrapper, .e-dialog-scroller, #messageBox").addClass("msg-box-height");  //For height adjustment purpose
            $(".message-box-btn-holder").addClass("msg-footer-height");
            parent.messageBox("su-user-1", window.Server.App.LocalizationContent.DatabaseSynchronization, result.Message, "success", function () {
                parent.onCloseMessageBox();
                hideWaitingPopup("server-app-container");
            });
            gridObj.clearSelection();
        ...
        error: function () {
            $("#messageBox_wrapper, .e-dialog-scroller, #messageBox").addClass("msg-box-height");  //For height adjustment purpose
            $(".message-box-btn-holder").addClass("msg-footer-height");
            parent.messageBox("su-user-1", window.Server.App.LocalizationContent.DatabaseSynchronization, window.Server.App.LocalizationContent.ExistingDatabaseSynchronizeFailed, "success", function () {
                parent.onCloseMessageBox();
            });
            hideWaitingPopup("server-app-container");
        }
