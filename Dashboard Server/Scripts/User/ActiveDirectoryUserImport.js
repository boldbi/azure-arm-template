        ...
        $.ajax({
            ...
            success: function (result) {
                    if (result.isUserLimitExceed != true) {
                        ...
                        var title = saveSelectedActiveDirectoryUserUrl.indexOf("azure") > 0 ? window.Server.App.LocalizationContent.AzureADUserImport : window.Server.App.LocalizationContent.ADUserImport;
                        if ($.type(result) == "object" && result.Count != 0) {
                            var message = "";
                            if (objectSelected.length != result.Count)
                                message += "<li>" + (parseInt(objectSelected.length) - parseInt(result.Count)) + window.Server.App.LocalizationContent.ADUsersImportSuccess + "</li>";

                            if (result.Count > 0)
                                message += "<li>" + result.Count + window.Server.App.LocalizationContent.IsDuplicateUser + " <br />(" + window.Server.App.LocalizationContent.IsUserEmailExist + ").</li>";

                            message = window.Server.App.LocalizationContent.ADUserImportSuccess + "<ul class='list-area'>" + message + "</ul>";
                            message += "<ul style='padding-left: 0px;list-style: none'><li>" + window.Server.App.LocalizationContent.ADDuplicateUser + "</li></ul>";
                            ...
                            parent.messageBox("su-user-1", title, objectSelected.length + window.Server.App.LocalizationContent.ADUsersImportSuccess, "success", function () {
                                parent.onCloseMessageBox();
                            });
                        ...
                        if (result.UserLicense.UsersLeft > 0) {
                            $("#limit-import-ad").show();
                            $("#limit-import-ad .licensed-user-count").html(result.UserLicense.Users);
                            $(".importad-allowed-count").html(result.UserLicense.UsersLeft);
                        } else {
                            $("#zero-user-acc .licensed-user-count").html(result.UserLicense.Users);
                            $("#zero-user-acc").show();
                        }
