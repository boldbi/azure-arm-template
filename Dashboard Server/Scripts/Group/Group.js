            ...
            if (data != "True") {
                ...
                doAjaxPost("POST", addGroupUrl, values, function (data, result) {
                    if (data != "null") {
                        ...
                        parent.messageBox("su-group-1", window.Server.App.LocalizationContent.AddGroup, window.Server.App.LocalizationContent.AddGroupSuccess, "success", function () {
                            var gridObj = parent.$("#Grid").data("ejGrid");
                            RefreshCurrentDataOfGroupList(gridObj);
                            parent.onCloseMessageBox();
                        });
                    } else {
                        parent.messageBox("su-group-1", window.Server.App.LocalizationContent.AddGroup, window.Server.App.LocalizationContent.AddGroupFail, "error", function () {
                            var gridObj = parent.$("#Grid").data("ejGrid");
                            RefreshCurrentDataOfGroupList(gridObj);
                            parent.onCloseMessageBox();
                        });
                ...
                $(".error-message").html(window.Server.App.LocalizationContent.IsGroupExist);
            }
        });
    }
});
...
$(document).on('click', ".delete", function (e) {
    ...
    var adminGroup = $(".group-delete-button").attr("data-groupid");
            ...
            function (data) {
                if (data.status) {
                    ...
                    if (gridName == "editgroup")
                        ...
                        if (data.status) {
                            parent.messageBox("su-delete", window.Server.App.LocalizationContent.DeleteGroup, window.Server.App.LocalizationContent.DeleteGroupSuccess, "success", function () {
                                parent.onCloseMessageBox();
                            });
                        } else {
                            parent.messageBox("su-delete", window.Server.App.LocalizationContent.DeleteGroup, window.Server.App.LocalizationContent.DeleteGroupFail, "error", function () {
                                parent.onCloseMessageBox();
                            });
                        }
                    }
...
function fnOnGroupGridActionComplete(args) {
    ...
    if (window.location.search.indexOf("action=addgroup") != -1 && isGroupGridFirstLoad) {
        isGroupGridFirstLoad = false;
        $("#new-group-button").trigger('click');
    }
}
         ...
         "POST", deleteFromGroupListUrl, values, function (data) {
             if (data.status) {
                 ...
                 if (groupcurrentValue > 0 || ad > 0 || azurecount > 0) {
                     var servergroupcount = groupcurrentValue > 0 ? groupcurrentValue + " " + window.Server.App.LocalizationContent.ServerGroups : "";
                     var activedirectorygroupcount = ad > 0 ? ad + " " + window.Server.App.LocalizationContent.AdGroups : "";
                     var azureadgroupcount = azurecount > 0 ? azurecount + " " + window.Server.App.LocalizationContent.AzureAdGroups : ""
                 }

                 if (groupcurrentValue > 0 && ad > 0) {
                     var servergroupcount = groupcurrentValue > 0 ? groupcurrentValue + " " + window.Server.App.LocalizationContent.ServerGroupsAnd : "";
                     var activedirectorygroupcount = ad > 0 ? ad + " " + window.Server.App.LocalizationContent.AdGroups : "";
                 }
                 if (ad > 0 && azurecount > 0) {
                     var activedirectorygroupcount = ad > 0 ? ad + " " + window.Server.App.LocalizationContent.AdGroupsAnd : "";
                     var azureadgroupcount = azurecount > 0 ? azurecount + " " + window.Server.App.LocalizationContent.AzureAdGroups : ""
                 }
                 ...
                 SuccessAlert(window.Server.App.LocalizationContent.DeleteMultipleGroup, servergroupcount + " " + activedirectorygroupcount + " " + azureadgroupcount + window.Server.App.LocalizationContent.DeleteSuccess, 7000);
                 parent.$("#group-count").html(currentVal);
                 parent.$("#group-count-text").val(currentVal);
                 ...
                 $("#groups-delete-confirmation").ejDialog("close");
                 WarningAlert(window.Server.App.LocalizationContent.DeleteMultipleGroup, window.Server.App.LocalizationContent.DeleteMultipleGroupFail, 7000);
             }
