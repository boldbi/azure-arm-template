...
$(document).ready(function () {
    ...
    }, window.Server.App.LocalizationContent.IsValidEndPoint);

    $.validator.addMethod("IsCustomEndpoint", function (value, element) {
        return IsCustomEndPoint(value, element);
    }, window.Server.App.LocalizationContent.IsCustomEndpoint);
    ...
    $("#blob_storage_form").validate({
        ...
        messages: {
            txt_accountname: {
                isRequired: window.Server.App.LocalizationContent.AccountnameValidator
            },
            txt_endpoint: {
                isRequired: window.Server.App.LocalizationContent.BlobEndpointValidator
            },
            txt_accesskey: {
                isRequired: window.Server.App.LocalizationContent.AcccessKeyValidator
            },
            txt_containername: {
                required: window.Server.App.LocalizationContent.ContainerValidator
            },
            txt_bloburl: {
                IsCustomEndpoint: window.Server.App.LocalizationContent.BlobUrlValidator,
                IsValidEndPoint: window.Server.App.LocalizationContent.IsValidBlobUrl,
                IsValidCustomEndPoint: window.Server.App.LocalizationContent.IsValidAccountName
            }
        }
    });
});
            ...
            doAjaxPost("POST", "../Administration/MigrateBlobs",
                ...
                function (result) {
                    if (typeof result.Data != "undefined") {
                        ...
                        hideWaitingPopup("page_content_Div");
                        $("#DisplayValidationMessage").html(window.Server.App.LocalizationContent.InValidBlob).show();
                    }
                }
            );
            return false;
